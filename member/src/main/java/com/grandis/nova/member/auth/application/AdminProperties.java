 ...
 * 평문을 넣고 "로그인만 안 되는" 상태와, 약한 cost 로 만든 해시가 조용히 쓰이는 상태를 막는다. cost 는 올릴 수 있다(D-4: 12 이상).
 */
@Validated
@ConfigurationProperties("admin")
public record AdminProperties(
        @NotBlank String username,
        @NotBlank @Pattern(regexp = "^\\$2[aby]\\$(1[2-9]|[23]\\d)\\$.{53}$", message = "admin.password-hash must be a bcrypt hash with cost >= 12") String passwordHash
) {
