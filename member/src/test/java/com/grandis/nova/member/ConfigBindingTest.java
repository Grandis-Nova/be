...
class ConfigBindingTest {
    ...
    static void fromExample(DynamicPropertyRegistry registry) {
        ...
        registry.add("jwt.secret", () -> "nova-test-signing-key-not-a-secret-xxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
    }
    ...
    @Test
    @DisplayName("auth.cookie.secure=true(예시 값) 컨텍스트에서 실제 Set-Cookie 헤더에 Secure·HttpOnly·SameSite=Lax 가 실린다")
    void secureCookieFlagReachesSetCookieHeader() throws Exception {
        ...
        String setCookie = MockMvcBuilders.webAppContextSetup(context).addFilters(mdcContextFilter, springSecurityFilterChain).build()
        ...
        assertThat(setCookie).startsWith(AuthCookies.REFRESH_TOKEN + "=")
                .contains("Secure").contains("HttpOnly").contains("SameSite=Lax").contains("Path=" + AuthCookies.PATH).contains("Max-Age=0");
    }
    ...
    void authPropertiesBind() {
        ...
        assertThat(revocation.failClosedPaths()).hasSize(5).contains("/api/v1/session/refresh");
