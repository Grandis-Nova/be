...
@SpringBootTest(classes = ChainTestApp.class, properties = {
        ...
        "jwt.secret=test-secret-key-for-jwt-provider-32bytes",
...
class SecurityChainTest {
    ...
    void setUp() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
                .addFilters(mdcContextFilter, springSecurityFilterChain)   // 서블릿 필터 순서 그대로: traceId → 보안 체인
                .build();
    ...
    @Test
    @DisplayName("토큰 없이 보호 경로는 401 UNAUTHENTICATED 봉투 (traceId 포함, retryable=false)")
    ...
    @Test
    @DisplayName("실측: 없는 URL 은 404 RESOURCE_NOT_FOUND 봉투다")
    ...
    @Test
    @DisplayName("들어온 X-Trace-Id 는 그대로 봉투와 응답 헤더에 실린다")
    void traceIdIsEchoed() throws Exception {
        mvc.perform(post("/api/v1/reservations").header(MdcContextFilter.TRACE_ID_HEADER, "trace-abc"))
                .andExpect(status().isUnauthorized())
                .andExpect(header().string(MdcContextFilter.TRACE_ID_HEADER, "trace-abc"))
