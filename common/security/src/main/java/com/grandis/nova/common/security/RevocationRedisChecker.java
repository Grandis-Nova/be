...
/**
 * 06 §5 의 키 두 개를 MGET 한 번으로 읽는다. 요청당 Redis 왕복은 이 한 번뿐이다(06 §8).
 *
 * 판정:
 * - auth:revoked-sid:{sid} 가 있으면 폐기.
 * - auth:nbf:{subject} 가 있고 그 값(epoch 초) 이 토큰 iat 이상이면 폐기. 같은 초도 폐기다 — iat·exp 가 초 정밀도라
 *   제재와 같은 초에 발급된 토큰이 살아남지 않게 경계를 안쪽으로 둔다. 07 §1 단계 2 행은 이 기준으로 고쳤다.
 * - 값이 숫자가 아니거나 Instant 범위 밖이면 폐기로 본다. 우리가 쓴 키에 우리가 모르는 값이 있는 것은 정상이 아니다.
 *   이 갈래는 조회가 성공한 것이라 D-2 의 fail-open 을 타지 않고 그 회원의 모든 경로가 401 이 되며, 재로그인해도 새 iat 가
 ...
 * D-2 가 "MGET 두 키 중 하나만 실패" 를 실패로 센 이유가 그것이고, 아래 size != 2 검사가 그 경우를 받는다.
 * 클러스터에서의 실제 동작은 클러스터가 뜨는 환경에서 재야 한다(07 §2). 재시도·회로 차단기·로컬 캐시는 두지 않는다(D-2).
 */
@Component
