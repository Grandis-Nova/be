...
/**
 * {@link CurrentViewer} 를 채운다. 임시 구현 — common:security(#14)의 인증 필터가 들어오면 그쪽 주체로 바꾼다.
 *
 * common:security 도입 시: {@link #current()} 를 NovaAuthentication 의 AuthenticatedPrincipal(role · customerId())로
 * 읽도록 바꾼다. 지금은 이름을 회원 id 로 파싱한다. common:security 의 CurrentCustomerId 리졸버와 판정이 갈리지 않게
 * 같은 주체에서 읽어야 한다.
 */
