...
class CatalogEntityMappingTest {
    ...
    void productAndOptionRoundTrip() {
        ...
        assertThat(optionRow.get("combination_key")).isNull();
        ...
        assertThat(options.findByProductIdOrderById(product.getId())).singleElement()
                .satisfies(o -> {
                    ...
                    assertThat(o.getCombinationKey()).isNull();
                });
    }
    ...
    @Test
    @DisplayName("상품은 비공개로 시작하고 공개 전환은 한 메서드로만 바뀐다")
    void productStartsHiddenUntilPublished() {
        Product product = products.saveAndFlush(Product.register(fixtures.category(), SaleMode.IN_STOCK,
                "Nova Book", BigDecimal.ZERO, null, null, false, BigDecimal.ZERO));
        assertThat(jdbcTemplate.queryForObject("SELECT visible FROM products WHERE id = ?", Boolean.class, product.getId()))
                .isFalse();

        product.changeVisibility(true);
    ...
    void imageRoundTrip() {
        Long productId = fixtures.product("IN_STOCK", "ACTIVE");
        ProductImage first = images.saveAndFlush(ProductImage.of(productId, ImageKind.GALLERY, "블랙", 0, "https://img/1.jpg", true));
        images.saveAndFlush(ProductImage.of(productId, ImageKind.GALLERY, "블랙", 1, "https://img/2.jpg", false));
        images.saveAndFlush(ProductImage.of(productId, ImageKind.DETAIL, "spec", 0, "https://img/3.jpg", true));

        assertThat(images.findByProductIdOrderByKindAscBundleKeyAscPositionAsc(productId))
                .extracting(ProductImage::getUrl)
                .containsExactly("https://img/3.jpg", "https://img/1.jpg", "https://img/2.jpg");
