 ...
 * 축이 없는 상품의 옵션({@link #standalone})은 키가 null 이고 NULL 끼리는 UNIQUE 가 안 걸린다.
...
public class ProductOption extends BaseEntity {
    ...
    /**
     * 축이 없는 상품에만, 상품당 하나. 조합 키 · JSON 은 비어 있다.
     * 키가 NULL 이라 DB UNIQUE 가 중복을 못 막으므로(NULL 끼리는 통과) 부르는 서비스가 상품의 축 · 기존 옵션을 보고 하나만 만든다.
     * 축이 있는 상품에 부르면 선택 행이 없는 옵션이 생기고 같은 조합의 중복도 못 잡는다.
     */
    public static ProductOption standalone(Long productId, String sku, String title, BigDecimal price) {
        return new ProductOption(productId, sku, title, price, false, null, null, null);
    }
